...
class Post(models.Model):
    ...
    votes_count = models.PositiveIntegerField(
        verbose_name="Vote counts", default=0
    )
...
class Vote(models.Model):
    post = models.ForeignKey(
        Post, verbose_name="Post", on_delete=models.CASCADE, related_name="votes"
    )
    user = models.ForeignKey(
        User, verbose_name="User", on_delete=models.CASCADE, related_name="votes"
    )
...
class Comment(models.Model):
    post = models.ForeignKey(
        Post, verbose_name="Posts", on_delete=models.CASCADE, related_name="comments"
    )
    ...
    def __str__(self):
        return (
            f"Comment on {self.post.title}'s post. " f"Commented {self.comment_author}"
        )
